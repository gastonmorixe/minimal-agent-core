The `interleave-thinking` plugin lets the model interleave reasoning with its response by emitting `<ma::plugin::interleave-thinking>...</ma::plugin::interleave-thinking>` spans inside assistant text. Interleave thinking is not user-visible as it's removed from them, it's a space for the model to freely think, reflect and improve mid-course.
