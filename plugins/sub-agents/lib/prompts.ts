 ...
 * here, never hardcoded as a TypeScript literal (the repo-wide convention; see
 * `src/prompts.ts`). This is the one module in the plugin's spawn path that
 * touches disk for prose, so `spawn-plan.ts` and `library.ts` stay pure: they
 * receive already-rendered text.
 *
 * @module sub-agents/lib/prompts
 */

import { loadPromptText, promptPath, renderPrompt } from "../../../src/prompts.ts"
