...
export interface SpawnInput {
  ...
  /** Resolved model id the child runs. */
...
/**
 * Compose the child prompt. When a specialization preamble is present it is
 * framed ABOVE the task so the worker reads its role first, then the concrete
 * objective. (A dedicated `--append-system-prompt` flag would be cleaner; this
 * works today without a core change.)
 */
export function composePrompt(task: string, systemPreamble?: string): string {
  const t = task.trim()
...
export function buildSpawnPlan(input: SpawnInput): Result<SpawnPlan> {
  ...
  if (input.model.trim().length === 0) return err("model is empty")
  if (!Number.isInteger(input.depth) || input.depth < 1) {
    return err(`depth must be a positive integer, got ${input.depth}`)
  }

  const prompt = composePrompt(input.task, input.systemPreamble)
