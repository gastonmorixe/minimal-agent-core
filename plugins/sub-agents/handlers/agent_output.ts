...
export default async function agentOutput(ctx: TUIContext): Promise<TUIResult> {
  ...
  if (!id) return { kind: "tool_result", content: "AgentOutput: an `id` is required.", is_error: true }

  const store = storeFromCtx(ctx)
  if (!store) return { kind: "tool_result", content: "AgentOutput: no session id available.", is_error: true }
  const rec = store.get(id)
  if (!rec) return { kind: "tool_result", content: `AgentOutput: unknown sub-agent "${id}".`, is_error: true }
  ...
  return {
    ...
    display: tail.length > 0 ? tail.map((l) => color(true, ANSI.DGRAY, l)).join("\n") : color(true, ANSI.DIM, "(no activity yet)"),
  }
}
