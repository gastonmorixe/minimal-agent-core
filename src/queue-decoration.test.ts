...
describe("buildQueueDecorationLines", () => {
  ...
  it("three entries at the cap: three numbered rows, last is ╰", () => {
  ...
  it("overflow (>3): all visible rows are ┊, tail carries ╰ ... more", () => {
    const lines = buildQueueDecorationLines(["a", "b", "c", "d", "e"])
    expect(lines.length).toBe(5) // header + 3 item rows + 1 elision tail
    expect(noAnsi(lines[0])).toBe("  ⏳ 5 queued")
    expect(noAnsi(lines[1])).toBe("  ┊  1 a")
    expect(noAnsi(lines[2])).toBe("  ┊  2 b")
    expect(noAnsi(lines[3])).toBe("  ┊  3 c")
    expect(noAnsi(lines[4])).toBe("  ╰  ... and 2 more")
  })
  ...
  it("truncates long previews with (+Nch) hint, preserves prefix", () => {
    ...
    // Hint format from truncate-hint.ts is "...(+50ch)" or similar.
  ...
  it("item-row prefix: 2 spaces + glyph + 2 spaces + num + space (5 cells + num)", () => {
  ...
  it("dims the glyph and content wrappers (ESC[2m … ESC[22m)", () => {
    ...
    // Header has `⏳` wrapped in faintWhite (dim+white) and "1 queued"
    // in plain dim. The item row has glyph + "N preview" both dim.
  ...
  it("cap constant matches the visible-rows behavior", () => {
    // Belt-and-braces: tightening the cap should not change the
    // overflow-tail format. Tests above are written against the
    // documented cap of 3, so this guards future tweaks.
    expect(QUEUE_MAX_VISIBLE_ITEMS).toBe(3)
  })
