...
describe("executeTool — universal clamp wiring", () => {
  it("Read on a huge file emits a structured truncation notice with real totals", () => {
    ...
    const r = executeTool("Read", { file_path: path })
  ...
  it("Read with offset reports cut line relative to offset", () => {
    ...
    const r = executeTool("Read", { file_path: path, offset: 200 })
    const m = r.content.match(/cut at byte \d+, line (\d+)/)
    expect(m).not.toBeNull()
    expect(Number(m![1])).toBeGreaterThan(200)
  })

  it("Read on a small file does NOT emit a notice", () => {
  ...
  it("Grep with massive output gets clamped with totals known", () => {
    const big = join(dir, "grepme.txt")
    writeFileSync(big, "match\n".repeat(MAX_TOOL_OUTPUT_LINES * 3))
    const r = executeTool("Grep", {
      pattern: "match",
      path: big,
      output_mode: "content",
  ...
  it("Bash output clamped with known totals (spawnSync buffered)", () => {
    const r = executeTool("Bash", {
  ...
  it("Edit is NOT clamped — display channel preserved intact", () => {
  ...
  it("Write is NOT clamped — display channel preserved intact", () => {
    const path = join(dir, "write.txt")
    const r = executeTool("Write", { file_path: path, content: "hi\n" })
    expect(r.content).not.toContain("[truncated:")
    expect(r.display).toBeDefined()
  })

  it("preserves is_error while clamping", () => {
    const r = executeTool("Bash", {
  ...
  it("small Bash output is not annotated", () => {
    const r = executeTool("Bash", { command: "echo hello" })
