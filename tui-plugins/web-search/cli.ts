 ...
 *
 * Run:
 *   bun run tui-plugins/web-search/cli.ts <query> [flags]
 *
 * Flags (all optional):
...
Examples:
  bun run tui-plugins/web-search/cli.ts "rust async runtime"
  bun run tui-plugins/web-search/cli.ts "EU AI act" -t news -L 5
  bun run tui-plugins/web-search/cli.ts "topic" -f json | jq '.hits[0].url'
`
