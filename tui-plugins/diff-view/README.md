...
`<tui::diff>` inline tag, so the model can either:
...
| Inline tag | `<tui::diff>` | `handlers/inline_diff.ts` |
...
The inline tag works correctly when a `<tui::diff>` opens in one streamed
