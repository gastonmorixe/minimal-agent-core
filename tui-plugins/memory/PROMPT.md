...
per-session scratchpad. Two write paths (the inline `<tui::memory>` tag
...
- **Saves still work the same way.** The inline `<tui::memory>` tag
...
still rides every turn as a `<short-term-memory>` attachment, so what
...
Short-term entries appear in your context as `<short-term-memory>` at
