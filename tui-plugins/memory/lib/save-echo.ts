 ...
 *           │                              <memory-saved scope="…" id="…">…</memory-saved>
 *
 ...
 *
 * Emitted by:
 *   - the inline-tag handler (`<tui::memory …>` save)
...
export class SaveEchoCollector {
   ...
   * `<memory-saved scope="…" id="…"[ evicted="N"]>preview</memory-saved>`.
...
export function renderEcho(p: MemorySavedPayload): string {
  ...
  // child tag inside `<memory-saved>…</memory-saved>`. We don't bother
  ...
  return `<memory-saved scope="${p.scope}" id="${escapeAttr(p.id)}"${evicted}>${safe}</memory-saved>`
}
